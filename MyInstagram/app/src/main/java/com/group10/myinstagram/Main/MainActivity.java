...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnInrange.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                mInrangeAdapter = new InRangePhotoListAdapter(mContext, R.layout
                        .layout_inrange_listitem, mInrangePhotos);
    ...
    private void getFollowing() {
        ...
        Query query = reference.child(getString(R.string.dbname_following)).child(FirebaseAuth
                .getInstance().getCurrentUser().getUid());
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot singleSnapshot : dataSnapshot.getChildren()) {
                    Log.d(TAG, "onDataChange: found user: " + singleSnapshot.child(getString(R
                            .string.field_user_id)).getValue());

                    mFollowing.add(singleSnapshot.child(getString(R.string.field_user_id))
                            .getValue().toString());
                }
    ...
    private void getPhotos() {
        ...
        for (int i = 0; i < mFollowing.size(); i++) {
            final int count = i;
            Query query = reference.child(getString(R.string.dbname_user_photos)).child
                    (mFollowing.get(i)).orderByChild(getString(R.string.field_user_id)).equalTo
                    (mFollowing.get(i));
            query.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    for (DataSnapshot singleSnapshot : dataSnapshot.getChildren()) {
                        ...
                        Map<String, Object> objectMap = (HashMap<String, Object>) singleSnapshot
                                .getValue();

                        photo.setCaption(objectMap.get(getString(R.string.field_caption))
                                .toString());
                        photo.setTags(objectMap.get(getString(R.string.field_tags)).toString());
                        photo.setPhoto_id(objectMap.get(getString(R.string.field_photo_id))
                                .toString());
                        photo.setUser_id(objectMap.get(getString(R.string.field_user_id))
                                .toString());
                        photo.setDate_created(objectMap.get(getString(R.string
                                .field_date_created)).toString());
                        photo.setImage_path(objectMap.get(getString(R.string.field_image_path))
                                .toString());
                        if (objectMap.get(getString(R.string.field_latitude)) != null &&
                                objectMap.get(getString(R.string.field_longitude)) != null) {
                            photo.setLatitude(Double.parseDouble(objectMap.get(getString(R.string
                                    .field_latitude)).toString()));
                            photo.setLongitude(Double.parseDouble(objectMap.get(getString(R
                                    .string.field_longitude)).toString()));
                        } else {
                            photo.setLatitude(0);
                        ...
                        for (DataSnapshot dSnapshot : singleSnapshot.child(getString(R.string
                                .field_likes)).getChildren()) {
                            Log.d(TAG, "onDataChange: main activity: load likes:" + dSnapshot
                                    .getValue(Like.class).getUsername());
                            Like like = new Like();
                        ...
                        for (DataSnapshot dSnapshot : singleSnapshot.child(getString(R.string
                                .field_comments)).getChildren()) {
                            Comment comment = new Comment();
                            comment.setUser_id(dSnapshot.getValue(Comment.class).getUser_id());
                            comment.setComment(dSnapshot.getValue(Comment.class).getComment());
                            comment.setDate_created(dSnapshot.getValue(Comment.class)
                                    .getDate_created());
    ...
    private void displayPhotos(int sortMode) {
        if (sortMode == 0) {
            ...
            if (mPhotos != null) {
                ...
                mAdapter = new UserfeedListAdapter(mContext, R.layout.layout_userfeed_listitem,
                        mPhotos);
            ...
            if (mPhotos != null) {
                Collections.sort(mPhotos, new Comparator<Photo>() {
                    @Override
                    public int compare(Photo o1, Photo o2) {
                        double dis1 = Math.abs(getDistance(myLongitude, myLatitude,
                                o1.getLongitude(), o1.getLatitude()));
                        double dis2 = Math.abs(getDistance(myLongitude, myLatitude,
    ...
    private void updateLocation(FirebaseUser user) {
        if (checkPermissionArray(Permissions.PERMISSIONS)) {
            ...
            if (location == null) {
                ...
                mReference.child(getString(R.string.dbname_users)).child(user.getUid()).child
                        (getString(R.string.field_longitude)).setValue(37.8136);
                mReference.child(getString(R.string.dbname_users)).child(user.getUid()).child
                        (getString(R.string.field_latitude)).setValue(144.9631);
                UserSuggest userSuggest = new UserSuggest(this, 37.8136, 144.9631);
            } else {
                DatabaseReference mReference = FirebaseDatabase.getInstance().getReference();
                mReference.child(getString(R.string.dbname_users)).child(user.getUid()).child
                        (getString(R.string.field_longitude)).setValue(location.getLongitude());
                mReference.child(getString(R.string.dbname_users)).child(user.getUid()).child
                        (getString(R.string.field_latitude)).setValue(location.getLatitude());
                UserSuggest userSuggest = new UserSuggest(this, location.getLongitude(), location
                        .getLatitude());
            }
